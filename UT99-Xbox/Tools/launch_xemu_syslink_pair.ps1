param(
    ...
    [switch]$ClientFirst,
    # Accepted for old command lines; RAM mirror logging replaced UDP debug markers.
    [string]$DebugUdpHost
)
...
function Rebuild-SourceIso()
{
    ...
    elseif( Test-Path $fourPlayerStressPath )
    {
    ...
    if( $FourPlayerStress )
    {
        Write-Host "System Link four-player stress marker: enabled"
    }
    else
    {
        Write-Host "System Link four-player stress marker: disabled"
    }
}
...
$HostConfig = Write-XemuConfig $HostDir 'host' $HostHdd $HostEeprom $HostIso '127.0.0.1:9360' '127.0.0.1:9361' $UdpBackend.IsPresent
$ClientConfig = Write-XemuConfig $ClientDir 'client' $ClientHdd $ClientEeprom $ClientIso '127.0.0.1:9361' '127.0.0.1:9360' $UdpBackend.IsPresent
...
elseif( $McastBackend )
{
    $McastEndpoint = '230.0.0.1:9362'
    $hostNetwork = Enable-McastNetwork 4478 $McastEndpoint
    $clientNetwork = Enable-McastNetwork 4479 $McastEndpoint
