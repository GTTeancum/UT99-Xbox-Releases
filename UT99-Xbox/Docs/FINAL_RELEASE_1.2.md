...
`C:/Programming/GitHub/UT99-Xbox-Releases` is the retained local checkout and build location. Before retiring `UnrealTournament_1.40`, its final build, console conversion inputs, Halo conversion/reference inputs, local SDK references, Xbox music, packaging inputs, final ZIP/checksum, and Git backup were moved into the retained checkout at the same relative paths. All 16,397 files (6.54 GiB) passed SHA-256 verification after the move. Disposable proof folders were left in the old checkout. The local file/hash inventory is `release_work/retained_inputs_move_manifest.json`; these local inputs remain ignored by Git.
